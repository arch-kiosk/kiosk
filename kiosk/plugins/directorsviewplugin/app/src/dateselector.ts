...
class DateSelector extends KioskStoreAppComponent {
    ...
    loadWorkDays(records: Array<AnyDict>, deletionRecords: Array<Array<string>>) {
        const workDays = new Set<number>(records.map((r) => {
            return DateTime.fromISO(r["data"]).toMillis();
        }))
        deletionRecords.forEach(r => {
            try {
                workDays.add(DateTime.fromISO(r[0]).toMillis());
            } catch {}
        })
        this.workDays = Array.from(workDays)
        this.workDays.sort()
    }

    jumpToWorkday(evt: MouseEvent) {
        const el = <HTMLElement>evt.currentTarget
        const selectedDate = DateTime.fromJSDate(this.selected_date, { zone: "utc" })
        const selectedDateMs = selectedDate.toMillis()
        let nextIndex: number

        switch (el.id) {
            case "bt-prev-workday": nextIndex = _.sortedLastIndex(this.workDays, selectedDateMs) -2;break;
            case "bt-next-workday": nextIndex = _.sortedIndex(this.workDays, selectedDateMs) + 1;break;
    ...
    apiRender() {
        ...
        for (let d = start_date; d <= end_date; d++) {
            ...
            newDate.setDate(d);
            dateList.push(DateTime.fromJSDate(newDate, {zone: "utc"}));
        }
        ...
        return html`
            <div class="date-selector">
                ...
                ${dateList.map((d) => html`
                    ${selectedDate.isValid && d && d.toMillis() === selectedDate.toMillis()
                        ? html`
                        ...
                        : html`
                            <div date="${d.toMillis()}" class="date-selector-date-inactive ${this.workDays?.includes(d.toMillis())?'active-day':''}" @click="${this.dateClicked}">
