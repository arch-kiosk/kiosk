...
// @ts-ignore
import narrativeWidgetStyle from './component-narrative-widget.sass';
...
class NarrativeWidget extends connect(store)(LitElement) {
    ...
    stateChanged(state: State) {
        ...
        if (state.constants.length > 0 && !this.record_type_names) {
            ...
            if (!changed) this.requestUpdate().then(r => {
            })
        }
    }
        ...
        return html`${this.api_url !== "" && this.login_token !== ""
            ? html`
                    <div class="narrative-widget">
                        <div class="headline">
                            <p>${this.narratives.length} Narrative(s)</p>
                        </div>
                        <div class="controls">
