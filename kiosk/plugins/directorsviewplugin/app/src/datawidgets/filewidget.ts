...
// @ts-ignore
import widgetStyle from './component-file-widget.sass';
...
class FileWidget extends connect(store)(LitElement) {
    ...
    fetchResolutions() {
        if (!this.selected_resolution) {
            if (this.login_token && this.api_url) {
                fetchFromApi(this.api_url, this.login_token, "files/resolutions",
                    {
                        method: "GET",
    ...
    get_conditions(us_date: string, identifier: string, member: string) {
        if (identifier || member) {
            ...
            if (identifier) conditions.push(`equals(identifier, '${identifier}')`)
    ...
    stateChanged(state: State) {
        ...
        if ("dateSelector" in state.selectors && "filewidget" in state.dataviews) {
            ...
            if (state.constants.length > 0 && !this.record_type_names) {
                ...
                if (!doFetch) this.requestUpdate().then(r => {
                })
            }
        }
    }
