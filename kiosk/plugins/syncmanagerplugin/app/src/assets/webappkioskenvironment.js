...
function kioskToggleCollapsible(jqCollapsibleButton) {
  ...
  if (coll && !coll.hasClass("kiosk-collapsible")) {
    coll = coll.parent();
  }
...
function initCollapsibles() {
  ...
  $(".kiosk-collapsible-clicker").click((This) => {
    kioskToggleCollapsible($(This.currentTarget));
  });
}
...
function kioskSendAjaxCommand(type, jqBt, url, jsonData, onSuccess, onFailure) {
  // inject_button_loader(jqBt, "#dddddd");
  kioskDeleteAllToasts();
  $.ajax({
    type: type,
    dataType: "json",
    ...
    .fail((xhr, status, errorThrown) => {
      remove_button_loader(jqBt);
      if (onFailure) onFailure(errorThrown,null);
    });
...
function kioskInitAwesompleteComboBox(elementId, dataListId, buttonId="", awesomepleteWidth="") {
  ...
  let combobox = new Awesomplete(input, {list: dataListId, minChars: 0});
...
function kioskStartWhenReady(func, fail_func = null, wait_cycles = 5, _first_call=true) {
  if (_first_call) {
