"""
    Plugin with the class KioskPhysicaljp2File that handles jp2 Images
"""
import logging
...
# *******************************************+
# KioskPhysicaljp2File
# ********************************************
class KioskPhysicaljp2File(KioskPhysicalImageFile):
    """ KioskPhysicalFile sub class that supports the jp2 format with extensions jp2 and JPG """

    supported_file_formats = [("JP2", "JPEG 2000 Image Format", ["jp2", "jpf", "jpx"], None),
    ...
    def get_exif_dict(self, img: Image):
        """
        read exif data from jp2 (well, it works with Wand in general).
        :param img:
        :return:
        """
        exif = {}
    ...
    def _fix_rotation(self, img: Image, representation):
        ...
        try:
            logging.debug(f"{self.__class__.__name__}._fix_rotation: No rotation fix for jp2s")
            return img
...
# ************************************************************************
class Pluginjp2FileHandling(SynchronizationPlugin):
    _plugin_version = 1.0
    ...
    def all_plugins_ready(self):
        ...
        if app:
            ...
            logging.debug("Pluginjp2FileHandling: plugin registered")
        else:
            logging.error("Pluginjp2FileHandling: plugin could not be registered due to no app.")
            return False
    ...
    def register_types(self, app: Synchronization, config: SyncConfig):
        ...
        KioskPhysicaljp2File.register(app.type_repository, add_format)
