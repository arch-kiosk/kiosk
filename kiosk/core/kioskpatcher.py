...
class KioskPatcher:
    ...
    def read_patch_file(self, patch_file=None) -> Tuple[bool, str]:
        try:
            ...
            err_msg = f"{self.__class__.__name__}.read_patch_file: Error when opening {self.patch_path_and_filename}: {repr(e)}"
            logging.error(err_msg)
            return False, err_msg
    ...
    def _check_min_version(self):
        try:
            ...
            if min_version:
                if kioskstdlib.cmp_semantic_version(min_version, kioskglobals.kiosk_version) > 0:
                    err_msg = f"{self.__class__.__name__}.patch_can_run: " \
                              f"Patch {self.id} expects kiosk to be at least version {min_version}, " \
                              f"but Kiosk is on version {kioskglobals.kiosk_version}."
                    logging.error(err_msg)
                    return False, err_msg
            else:
                logging.info(f"{self.__class__.__name__}.patch_can_run: Patch {self.id} "
    ...
    def _check_required_version(self):
        try:
            ...
            if required_version:
                if kioskstdlib.cmp_semantic_version(required_version, kioskglobals.kiosk_version) != 0:
                    err_msg = f"{self.__class__.__name__}.patch_can_run: " \
    ...
    def start_python_script(self, script_file: str):
        ...
        try:
            ...
            if hasattr(module, "patch_script_main"):
                return module.patch_script_main(self.cfg, self.transfer_dir, kioskglobals.kiosk_version)
            else:
    ...
    def run_unpackkiosk(self) -> [bool, str]:
        ...
        if not kioskstdlib.to_bool(kioskglobals.get_development_option("test_patch").lower() == "true"):
            ...
            if kioskglobals.is_development_system():
                err_msg = f"{self.__class__.__name__}.run_unpackkiosk: " \
    ...
    def start_unpackkiosk_sync(self, unpackkiosk_dir, unpackkiosk_file, unpackkiosk_parameters):
        ...
        try:
            cmdline = ["python", os.path.join(unpackkiosk_file), self.transfer_dir, self.cfg.base_path]
            cmdline.extend(unpackkiosk_parameters)
            result = subprocess.run(cmdline, cwd=unpackkiosk_dir, stdout=subprocess.DEVNULL)
            rc = result.returncode
